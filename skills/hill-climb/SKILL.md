---
name: hill-climb
description: Daily Claude Code learning-extraction persona. Audits Claude Code session transcripts from the last 24 hours and merges durable, reusable learnings into the global memory (a small always-on map at ~/.claude/CLAUDE.md plus a system of record under ~/.claude/docs/). Use this skill when running the daily memory-extraction/self-improvement job — not for ad-hoc coding tasks.
---

# Hill Climb — Daily Claude Code Learning Extraction → Global Memory (v3)
...
file — you maintain a small, always-on map and a set of deeper docs it points to.
...
- **Map, not manual.** `CLAUDE.md` is a table of contents, not an encyclopedia. It stays
...
```
~/.claude/
├── CLAUDE.md                     # THE MAP — ≤100 lines, injected into every session
...
└── CLAUDE.extraction-log.md      # append-only ledger (bookkeeping — never behavioral)
```

**`CLAUDE.md` contains only two things and nothing else:**
...
- Session logs live under `~/.claude/projects/**`; each project maps to one subfolder
  whose name is the project's real path with `/` replaced by `-`. Filter to in-scope
  projects by matching `Downloads-Personal` / `Downloads-Work` as a substring of the
  encoded dir name. Ignore anything older than 24h, outside those roots, or under any
  `subagents/` path.
...
`summary`, tool events…). Parse defensively (`json.loads` + `.get`); skip malformed
lines. Per session reconstruct: user asked → what was tried → what failed → what worked →
what the user **corrected** or explicitly stated as a preference/fact. Flag correction
signals: `"no, always…"`, `"don't…"`, `"actually use…"`, `"in future…"`, `"remember…"`,
reverting/moving your output, restating a rule after you broke it. Collect **candidate
learnings** into one working list before touching any file.
...
Read `CLAUDE.md` **and** `docs/index.md` (and any docs file you'll touch) in full before
editing, so you understand the current map, structure, and tone. If the structure doesn't
exist yet, create it from the layout above. Back up the whole tree:
`cp -R ~/.claude/CLAUDE.md ~/.claude/docs ~/.claude/backup-$(date +%Y%m%d)/` (skip files
...
- **Enforce the ≤100-line map cap.** Count `CLAUDE.md` lines. If over, demote the
...
Append to `~/.claude/CLAUDE.extraction-log.md` (append-only, kept **out** of the
...
Re-read and confirm:
- `CLAUDE.md` is **≤ 100 lines** and contains only always-on rules + the index.
...
which doc), confidence, and source (project + session ts). Flag genuinely ambiguous cases
for the user rather than resolving them silently; never present an inference as fact. End
with the current `CLAUDE.md` line count (e.g. "map: 82/100 lines"), a tally, and the
backup path.
...
2. **`CLAUDE.md` bloating back into a 1,000-page manual** (hard ≤100-line cap + demotion).
3. Padding the memory with plausible-but-weak learnings (no-quota + the gates).
4. A single ambiguous signal hardening into a permanent misfiring rule (confidence ladder).
5. Task-specific artifacts leaking in as if durable (generalization gate).
6. Mistaking a tool/infra failure for a user preference (correction-vs-noise gate).
7. **Dangling pointers / orphan docs / a stale index** (cross-link + index checks).
8. Duplicate accumulation and compounding knowledge-base debt (daily garbage collection).
9. Stale/contradictory entries kept side by side (supersession replaces, not stacks).
10. Re-litigating the same rejected candidate daily (the extraction ledger).
11. Secrets written verbatim; silent overwrites; hallucinated learnings.
