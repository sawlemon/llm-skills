...
function enforceEvidenceGate(analysis, transcript) {
  ...
  for (const learning of analysis.learnings) {
    ...
    const verified = typeof quote === "string" && quote.length > 0 && sourceText && sourceText.includes(quote);
    if (verified) {
      kept.push(learning);
    } else {
  ...
  const confirmedIds = new Set(
    kept.filter((l) => l.confidence === "confirmed").map((l) => l.evidence.messageId + "|" + l.evidence.quote)
  );
  ...
  return {
    learnings: kept,
    rejected: [...analysis.rejected, ...droppedAsUnverified.map((d) => ({ candidate: d.claim, reason: d.reason }))],
...
export async function analyze({ currentPrompt, transcript, model }) {
  ...
  try {
    ...
    throw new AnalysisError(`Analyzer did not return valid JSON: ${err.message}\nRaw: ${content.slice(0, 500)}`);
  }
